defmodule SymphonyV2.Agents.SafehouseTest do
  ...
  describe "build_command/3" do
    test "builds correct command for claude_code" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:claude_code, "/workspace/task-1", prompt: "Fix the bug")
    ...
    test "builds correct command for codex" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:codex, "/workspace/task-1", prompt: "Add tests")
    ...
    test "builds correct command for gemini_cli" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:gemini_cli, "/workspace/task-1",
    ...
    test "builds correct command for opencode (no env vars, no skip flag)" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:opencode, "/workspace/task-1", prompt: "Fix it")
    ...
    test "includes read-only directories" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:claude_code, "/workspace/task-1",
                 prompt: "Do it",
                 read_only_dirs: ["/shared/libs", "/shared/config"]
               )
    ...
    test "deduplicates environment variables" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:claude_code, "/workspace/task-1",
                 prompt: "Do it",
                 env_vars: ["ANTHROPIC_API_KEY", "EXTRA"]
               )
    ...
    test "skip_permissions defaults to true" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:claude_code, "/workspace/task-1", prompt: "Do it")
    ...
    test "returns error for unknown agent type" do
      assert {:error, msg} =
               Safehouse.build_command(:nonexistent, "/workspace", prompt: "Do it")
  ...
  describe "build_command/3 — path safety" do
    test "rejects paths with null bytes" do
      assert {:error, msg} =
               Safehouse.build_command(:claude_code, "/workspace\0/evil", prompt: "Do it")
    ...
    test "rejects paths with semicolons" do
      assert {:error, _} =
               Safehouse.build_command(:claude_code, "/workspace; rm -rf /", prompt: "Do it")
    end

    test "rejects paths with pipes" do
      assert {:error, _} =
               Safehouse.build_command(:claude_code, "/workspace | cat /etc/passwd",
                 prompt: "Do it"
               )
    end
    ...
    test "rejects paths with ampersands" do
      assert {:error, _} =
               Safehouse.build_command(:claude_code, "/workspace & echo pwned", prompt: "Do it")
    end

    test "rejects paths with backticks" do
      assert {:error, _} =
               Safehouse.build_command(:claude_code, "/workspace/`whoami`", prompt: "Do it")
    end
    ...
    test "rejects paths with newlines" do
      assert {:error, _} =
               Safehouse.build_command(:claude_code, "/workspace\n/etc/passwd", prompt: "Do it")
    end

    test "rejects empty paths" do
      assert {:error, _} = Safehouse.build_command(:claude_code, "", prompt: "Do it")
    end
    ...
    test "rejects unsafe read-only paths" do
      assert {:error, msg} =
               Safehouse.build_command(:claude_code, "/workspace/task-1",
                 prompt: "Do it",
                 read_only_dirs: ["/safe/path", "/evil; rm -rf /"]
               )
    ...
    test "accepts paths with spaces" do
      assert {:ok, {"safehouse", args}} =
               Safehouse.build_command(:claude_code, "/my workspace/task 1", prompt: "Do it")
    ...
    test "accepts paths with dots and hyphens" do
      assert {:ok, _} =
               Safehouse.build_command(:claude_code, "/workspace/task-1/sub.dir", prompt: "Do it")
    end
  ...
  describe "build_command_list/3" do
    test "returns flat list with command as first element" do
      assert {:ok, list} =
               Safehouse.build_command_list(:claude_code, "/workspace/task-1", prompt: "Do it")
    ...
    test "returns error for unknown agent" do
      assert {:error, _} =
               Safehouse.build_command_list(:nonexistent, "/workspace", prompt: "Do it")
    end
  end
end
